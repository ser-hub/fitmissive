export default class WorkoutManager {
    ...
    $setTitle(title) {
        console.log(title)
        let carouselItem = this.$parentConatiner.parentNode.nextSibling
        if (carouselItem && title) {
            let titleNode = carouselItem.nextSibling.childNodes[1]
    ...
    $addNumericDropdown(parent, specificClass = '', min = 1, max = 1, selected = 1) {
        let numericDropdown = document.createElement('select')
    ...
    $addNumericDropdown2(parent, specificClass = '', min = 1, max = 1, selected = 1) {
        let dropdownWrapper = document.createElement('div')
        let dropdownButton = document.createElement('button')
        let dropdownOptions = documents.createElement('div')


        if (min < max) {
            for (let i = min; i <= max; i++) {
                let dropdownOption = document.create
            }
        }
    }

    $addStringDropdown(parent, specificClass = '', selected = '', data = []) {
        let stringDropdown = document.createElement('select')
